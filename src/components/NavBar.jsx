import '../components/NavBar.css'
import { Link } from 'react-router';

function NavBar(){
    return(
        <>
            <div id="navbar">
                <div className="menu-bar">
                    <ul>
                        <img style={{
                                height: "60px",
                                width: "60px",
                                marginTop: "11px",
                                marginLeft: "15px"
                            }}
                            src="data:image/png;base64,"
                            alt="MyntraLogo" />



                        <div id="nav-titles">
                            <li>
                                <span style={{padding:"26px 0px"}} className="menHover">MEN</span>
                                <div className="bcg"></div>
                             

                                {/* <div className="sub-menu-1">

                                    <div style={{marginTop:"-40px "}}>
                                        <ul>
                                            <li>
                                                <Link to="#">
                                                    <h4>Topwear</h4>
                                                </Link>
                                            </li>
                                            <li><Link to="./Product_Page/mens.html">T-Shirts</Link></li>
                                            <li><Link to="./Product_Page/mens.html">Casual Shirts</Link></li>
                                            <li><Link to="./Product_Page/mens.html">Sweatshirts</Link></li>
                                            <li><Link to="./Product_Page/mens.html">Jackets</Link></li>
                                            <li><Link to="./Product_Page/mens.html">Blazers & Coats</Link></li>
                                            <li><Link to="./Product_Page/mens.html">Suits</Link></li>
                                            <li><Link to="./Product_Page/mens.html">Rain Jackets</Link></li>
                                        </ul>
                                    </div>

                                    <div style={{marginTop:"-40px"}}>
                                        <ul>
                                            <li>
                                                <Link to="#">
                                                    <h4>Bottomwear</h4>
                                                </Link>
                                            </li>

                                            <li><Link to="#"> Jeans</Link></li>
                                            <li><Link to="#">Casual Trousers</Link></li>
                                            <li><Link to="#"> Formal Trousers</Link></li>
                                            <li><Link to="#"> Shortsr</Link></li>
                                            <li><Link to="#"> Track Pants & Joggers</Link></li>
                                        </ul>
                                    </div>

                                    <div style={{marginTop:"-40px"}}>
                                        <ul>
                                            <li>
                                                <Link to="#">
                                                    <h4>Footwear</h4>
                                                </Link>
                                            </li>
                                            <li><Link to="#"> Casual Shoes</Link></li>
                                            <li><Link to="#"> Sports Shoes</Link></li>
                                            <li><Link to="#"> Formal Shoes</Link></li>
                                            <li><Link to="#"> Sneakers</Link></li>
                                            <li><Link to="#"> Sandals & Floaters</Link></li>
                                            <li><Link to="#"> Flip Flops</Link></li>
                                            <li><Link to="#"> Socks</Link></li>
                                        </ul>
                                    </div>

                                    <div style={{marginTop:"-40px"}}>
                                        <ul>
                                            <li>
                                                <Link to="#">
                                                    <h4>Sports & Active Wear</h4>
                                                </Link>
                                            </li>
                                            <li><Link to="#">Sports Shoes</Link></li>
                                            <li><Link to="#"> Sports Sandals</Link></li>
                                            <li><Link to="#"> Active T-Shirts</Link></li>
                                            <li><Link to="#">Track Pants & Shorts</Link></li>
                                            <li><Link to="#"> Tracksuits</Link></li>
                                            <li><Link to="#"> Jackets & Sweatshirts</Link></li>
                                            <li><Link to="#"> Sports Accessories</Link></li>
                                            <li><Link to="#"> Swimwear</Link></li>
                                        </ul>
                                    </div>

                                    <div style={{marginTop:"-40px"}}>
                                        <ul>
                                            <li>
                                                <Link to="#">
                                                    <h4>Fashion Accessories</h4>
                                                </Link>
                                            </li>
                                            <li><Link to="#"> Wallets</Link></li>
                                            <li><Link to="#"> Belts</Link></li>
                                            <li><Link to="#"> Perfumes & Body Mists</Link></li>
                                            <li><Link to="#"> Trimmers</Link></li>
                                            <li><Link to="#">Ties, Cufflinks & Pocket Squares</Link></li>
                                            <li><Link to="#"> Accessory Gift Sets</Link></li>
                                            <li><Link to="#"> Caps & Hats</Link></li>
                                            <li><Link to="#"> Mufflers, Scarves & Gloves</Link></li>
                                            <li><Link to="#"> Rings & Wristwear</Link></li>
                                            <li><Link to="#"> Helmets</Link></li>
                                        </ul>
                                    </div>

                                    <div className="bcg"></div>


                                </div> */}


                            </li>

                            <li>
                                <span style={{padding:"26px 0px"}} id="wm">WOMEN</span>
                                <div className="bcg"></div>

                                {/* <div id="sb1" className="sub-menu-1" style={{marginLeft:"-135px"}}>
                                    <div style={{marginTop:"-40px"}}>
                                        <ul>
                                            <li>
                                                <Link to="#">
                                                    <h4>Indian & Fusion Wear</h4>
                                                </Link>
                                            </li>
                                            <li><Link to="./Product_Page/womens.html"> Kurtas & Suits</Link></li>
                                            <li><Link to="./Product_Page/womens.html"> Kurtis, Tunics & Tops</Link></li>
                                            <li><Link to="./Product_Page/womens.html"> Sarees</Link></li>
                                            <li><Link to="./Product_Page/womens.html"> Ethnic Wear</Link></li>
                                            <li><Link to="./Product_Page/womens.html"> Leggings, Salwars & Churidars</Link></li>

                                            <li><Link to="#"> Skirts & Palazzos</Link></li>
                                            <li><Link to="#"> Dress Materials</Link></li>
                                            <li><Link to="#"> Lehenga Cholis</Link></li>
                                            <li><Link to="#"> Dupattas & Shawls</Link></li>

                                            <li><Link to="#"> Jackets </Link></li>
                                        </ul>
                                    </div>
                                    <div style={{marginTop:"-40px"}}>
                                        <ul>
                                            <li>
                                                <Link to="#">
                                                    <h4>Western Wear</h4>
                                                </Link>
                                            </li>
                                            <li><Link to="#"> Dresses</Link></li>
                                            <li><Link to="#">Tops</Link></li>
                                            <li><Link to="#">Tshirts</Link></li>
                                            <li><Link to="#"> Jeans</Link></li>
                                            <li><Link to="#"> Trousers & Capris</Link></li>

                                            <li><Link to="#"> Co-ords</Link></li>
                                            <li><Link to="#"> Playsuits</Link></li>
                                            <li><Link to="#"> Jumpsuits</Link></li>
                                            <li><Link to="#"> Shrugs</Link></li>
                                            <li><Link to="#"> Sweaters & Sweatshirts</Link></li>
                                            <li><Link to="#">Jackets & Coats</Link></li>
                                            <li><Link to="#"> Blazers & Waistcoats</Link></li>
                                        </ul>
                                    </div>
                                    <div style={{marginTop:"-40px"}}>
                                        <ul>
                                            <li>
                                                <Link to="#">
                                                    <h4>Maternity</h4>
                                                </Link>
                                            </li>
                                            <br />
                                            <li>
                                                <Link to="#">
                                                    <h4>Sunglasses & Frames</h4>
                                                </Link>
                                            </li>
                                            <br />
                                            <li>
                                                <Link to="#">
                                                    <h4>Footwear</h4>
                                                </Link>
                                            </li>
                                            <li><Link to="#"> Flats</Link></li>
                                            <li><Link to="#"> Casual Shoes</Link></li>
                                            <li><Link to="#">Tshirts</Link></li>
                                            <li><Link to="#"> Heels</Link></li>
                                            <li><Link to="#"> Boots</Link></li>

                                         
                                        </ul>
                                    </div>
                                    <div style={{marginTop:"-40px"}}>
                                        <ul>
                                            <li>
                                                <Link to="#">
                                                    <h4>Lingerie & Sleepwear</h4>
                                                </Link>
                                            </li>
                                            <li><Link to="#">Bra</Link></li>
                                            <li><Link to="#"> Briefs</Link></li>
                                            <li><Link to="#"> Shapewear</Link></li>
                                            <li><Link to="#"> Sleepwear & Loungewear</Link></li>
                                            <li><Link to="#"> Swimwear</Link></li>

                                            <li><Link to="#"> Camisoles & Thermals</Link></li>
                                        </ul>
                                    </div>
                                    <div style={{marginTop:"-40px"}}>
                                        <ul>
                                            <li>
                                                <Link to="#">
                                                    <h4>Gadgets</h4>
                                                </Link>
                                            </li>
                                            <li><Link to="#">Smart Wearables</Link></li>
                                            <li><Link to="#"> Fitness Gadgets</Link></li>
                                            <li><Link to="#"> Headphones</Link></li>
                                            <li><Link to="#"> Speakers</Link></li>
                                        </ul>
                                    </div>



                                    <div className="bcg"></div>





                                </div> */}
                            </li>
                            <li>
                                <span style={{padding:"26px 0px"}} id="kd">KIDS</span>
                                <div className="bcg"></div>

                                {/* <div id="sb2" className="sub-menu-1" style={{marginLeft:"-228px"}}>
                                    <div className="bcg"></div>
                                    <div style={{marginTop:"-40px"}}>
                                        <ul>
                                            <li>
                                                <Link to="#">
                                                    <h4>Boys Clothing</h4>
                                                </Link>
                                            </li>
                                            <li><Link to="#"> T-Shirts</Link></li>
                                            <li><Link to="#"> Shirts</Link></li>
                                            <li><Link to="#"> Shorts</Link></li>
                                            <li><Link to="#"> Jeans</Link></li>
                                            <li><Link to="#"> Trousers</Link></li>

                                            <li><Link to="#"> Clothing Sets</Link></li>
                                            <li><Link to="#"> Ethnic Wear</Link></li>
                                            <li><Link to="#"> Track Pants & Pyjamas</Link></li>
                                            <li><Link to="#"> Jacket, Sweater & Sweatshirts</Link></li>

                                            <li><Link to="#"> Party Wear </Link></li>
                                            <li><Link to="#"> Innerwear & Thermals </Link></li>
                                           
                                            <li><Link to="#"> Value Packs </Link></li>
                                        </ul>
                                    </div>

                                    <div style={{marginTop:"-40px"}}>
                                        <ul>
                                            <li>
                                                <Link to="#">
                                                    <h4>Girls Clothing</h4>
                                                </Link>
                                            </li>
                                            <li><Link to="#"> Dresses</Link></li>
                                            <li><Link to="#"> Tops</Link></li>
                                            <li><Link to="#"> Tshirts</Link></li>
                                            <li><Link to="#"> Clothing Sets</Link></li>
                                            <li><Link to="#"> Lehenga choli</Link></li>

                                            <li><Link to="#"> Kurta Sets</Link></li>
                                            <li><Link to="#">Party wear</Link></li>
                                            <li><Link to="#"> Dungarees & Jumpsuits</Link></li>
                                            <li><Link to="#"> Skirts & shorts</Link></li>

                                            <li><Link to="#"> Tights & Leggings </Link></li>
                                            <li><Link to="#">Jeans, Trousers & Capris </Link></li>
                                            <li><Link to="#">Jacket, Sweater & Sweatshirts</Link></li>
                                      

                                        </ul>
                                    </div>
                                    <div style={{marginTop:"-40px"}}>
                                        <ul>
                                            <li>
                                                <Link to="#">
                                                    <h4>Footwear</h4>
                                                </Link>
                                            </li>
                                            <li><Link to="#"> Casual Shoes</Link></li>
                                            <li><Link to="#"> Flipflops</Link></li>
                                            <li><Link to="#"> Sports Shoes</Link></li>
                                            <li><Link to="#"> Flats</Link></li>
                                            <li><Link to="#"> Sandals</Link></li>

                                            <li><Link to="#"> Heels</Link></li>
                                            <li><Link to="#"> School Shoes</Link></li>
                                            <li><Link to="#"> Socks</Link></li>
                                        </ul>
                                    </div>
                                    <div style={{marginTop:"-40px"}}>
                                        <ul>
                                            <li>
                                                <Link to="#">
                                                    <h4>Infants</h4>
                                                </Link>
                                            </li>
                                            <li><Link to="#"> Bodysuits</Link></li>
                                            <li><Link to="#"> Rompers & Sleepsuits</Link></li>
                                            <li><Link to="#"> Clothing Sets</Link></li>
                                            <li><Link to="#"> Tshirts & Tops</Link></li>
                                            <li><Link to="#"> Dresses</Link></li>

                                            <li><Link to="#"> Bottom wear</Link></li>
                                            <li><Link to="#"> Winter Wear</Link></li>
                                            <li><Link to="#"> Innerwear & Sleepwear</Link></li>
                                            <li><Link to="#"> Infant Care</Link></li>
                                        </ul>
                                    </div>

                                    <div style={{marginTop:"-40px"}}>
                                        <ul>
                                            <li>
                                                <Link to="#">
                                                    <h4>Kids Accessories</h4>
                                                </Link>
                                            </li>
                                            <li><Link to="#"> Bags & Backpacks</Link></li>
                                            <li><Link to="#"> Jewellery & Hair accessory</Link></li>
                                            <li><Link to="#"> Sunglasses</Link></li>
                                            <li><Link to="#"> Masks & Protective Gears</Link></li>
                                            <li><Link to="#"> Caps & Hats</Link></li>
                                        </ul>
                                    </div>
                                </div> */}
                            </li>
                            <li>
                                <span style={{padding:"26px 0px"}} id="hl">HOME & LIVING</span>
                                <div className="bcg"></div>
                                {/* <div id="sb3" className="sub-menu-1" style={{marginLeft:"-295px"}}>
                                    <div className="bcg"></div>
                                    <div style={{marginTop:"-40px"}}>
                                        <ul>
                                            <li>
                                                <Link to="#">
                                                    <h4>Bed Linen & Furnishing</h4>
                                                </Link>
                                            </li>
                                            <li><Link to="#"> Bed Runners</Link></li>
                                            <li><Link to="#"> Mattress Protectors</Link></li>
                                            <li><Link to="#"> Bedsheets</Link></li>
                                            <li><Link to="#"> Bedding Sets</Link></li>
                                            <li><Link to="#"> Blankets, Quilts & Dohars</Link></li>

                                            <li><Link to="#"> Pillows & Pillow Covers</Link></li>
                                            <li><Link to="#"> Bed Covers</Link></li>
                                            <li><Link to="#">Diwan Sets</Link></li>
                                            <li><Link to="#">Chair Pads & Covers</Link></li>
                                            <li><Link to="#"> Sofa Covers</Link></li>
                                        </ul>
                                    </div>
                                    <div style={{marginTop:"-40px"}}>
                                        <ul>
                                            <li>
                                                <Link to="#">
                                                    <h4>Bath</h4>
                                                </Link>
                                            </li>
                                            <li><Link to="#">Bath Towels</Link></li>
                                            <li><Link to="#">Hand & Face Towels</Link></li>
                                            <li><Link to="#"> Beach Towels</Link></li>
                                            <li><Link to="#"> Towels Set</Link></li>
                                            <li><Link to="#"> Bath Rugs</Link></li>

                                            <li><Link to="#"> Bathroom Accessories </Link></li>
                                            <li><Link to="#"> Shower Curtains</Link></li>
                                            <li><Link to="#"> Bath Robes</Link></li>
                                        </ul>
                                    </div>
                                    <div style={{marginTop:"-40px"}}>
                                        <ul>
                                            <li>
                                                <Link to="#">
                                                    <h4>Home Décor</h4>
                                                </Link>
                                            </li>
                                            <li><Link to="#"> Plants & Planters</Link></li>
                                            <li><Link to="#"> Aromas & Candles</Link></li>
                                            <li><Link to="#"> Clocks</Link></li>
                                            <li><Link to="#"> Mirrors</Link></li>
                                            <li><Link to="#"> Wall Décor</Link></li>

                                            <li><Link to="#"> Festive Decor</Link></li>
                                            <li><Link to="#"> Pooja Essentials</Link></li>
                                            <li><Link to="#"> Wall Shelves</Link></li>
                                            <li><Link to="#"> Fountains</Link></li>
                                            <li><Link to="#"> Showpieces & Vases</Link></li>
                                            <li><Link to="#"> Ottoman</Link></li>
                                        </ul>
                                    </div>
                                    <div style={{marginTop:"-40px"}}>
                                        <ul>
                                            <li>
                                                <Link to="#">
                                                    <h4>Home Gift Sets</h4>
                                                </Link>
                                            </li>
                                            <br />
                                            <li>
                                                <Link to="#">
                                                    <h4>Kitchen & Table</h4>
                                                </Link>
                                            </li>
                                            <li><Link to="#"> Table Runners</Link></li>
                                            <li><Link to="#"> Dinnerware & Serveware</Link></li>
                                            <li><Link to="#"> Cups and Mugs</Link></li>
                                            <li><Link to="#"> Bakeware & Cookware</Link></li>
                                            <li><Link to="#"> Kitchen Storage & Tools</Link></li>

                                            <li><Link to="#"> Bar & Drinkware</Link></li>
                                            <li><Link to="#"> Table Covers & Furnishings</Link></li>
                                        </ul>
                                    </div>
                                    <div style={{marginTop:"-40px"}}>
                                        <ul>
                                            <li>
                                                <Link to="#">
                                                    <h4>Brands</h4>
                                                </Link>
                                            </li>
                                            <li><Link to="#"> H&M</Link></li>
                                            <li><Link to="#"> Marks & Spencer</Link></li>
                                            <li><Link to="#"> Home Centre</Link></li>
                                            <li><Link to="#">Spaces</Link></li>
                                            <li><Link to="#">D'Decor</Link></li>

                                            <li><Link to="#">Story@Home</Link></li>
                                            <li><Link to="#"> Raymond Home</Link></li>
                                            <li><Link to="#"> Trident</Link></li>
                                            <li><Link to="#"> Cortina</Link></li>
                                            <li><Link to="#"> Random</Link></li>
                                            <li><Link to="#"> Ellementry</Link></li>
                                            <li><Link to="#"> ROMEE</Link></li>
                                            <li><Link to="#"> SEJ by Nisha Gupta</Link></li>
                                        </ul>
                                    </div>
                                </div> */}
                            </li>
                            <li>
                                <span style={{padding:"26px 0px"}} id="beauty-tag">BEAUTY</span>
                                <div className="bcg"></div>
                                {/* <div id="sb4" className="sub-menu-1" style={{marginLeft:"-448px "}}>
                                    <div className="bcg"></div>
                                    <div style={{marginTop:"-40px"}}>
                                        <ul>
                                            <li>
                                                <Link to="#">
                                                    <h4>Makeup</h4>
                                                </Link>
                                            </li>
                                            <li><Link to="#">Lipstick</Link></li>
                                            <li><Link to="#"> Lip Gloss</Link></li>
                                            <li><Link to="#"> Lip Liner</Link></li>
                                            <li><Link to="#"> Mascara</Link></li>

                                            <li><Link to="#"> Eyeliner</Link></li>
                                            <li><Link to="#"> Kajal</Link></li>
                                            <li><Link to="#"> Eyeshadow</Link></li>
                                            <li><Link to="#"> Foundation</Link></li>
                                            <li><Link to="#"> Primer</Link></li>
                                            <li><Link to="#"> Concealer</Link></li>
                                            <li><Link to="#"> Compact</Link></li>
                                            <li><Link to="#"> Nail Polish</Link></li>
                                        </ul>
                                    </div>
                                    <div style={{marginTop:"-40px"}}>
                                        <ul>
                                            <li>
                                                <Link to="#">
                                                    <h4>Skincare, Bath & Body</h4>
                                                </Link>
                                            </li>
                                            <li><Link to="#"> Face Moisturiser</Link></li>
                                            <li><Link to="#"> Cleanser</Link></li>
                                            <li><Link to="#"> Masks & Peel</Link></li>
                                            <li><Link to="#"> Sunscreen</Link></li>

                                            <li><Link to="#"> Serum</Link></li>
                                            <li><Link to="#"> Face Wash</Link></li>
                                            <li><Link to="#"> Eye Cream</Link></li>
                                            <li><Link to="#"> Lip Balm</Link></li>
                                            <li><Link to="#"> Body Lotion</Link></li>
                                            <li><Link to="#"> Body Wash</Link></li>
                                            <li><Link to="#"> Body Scrub</Link></li>
                                            <li><Link to="#"> Hand Cream</Link></li>
                                        </ul>
                                    </div>
                                    <div style={{marginTop:"-40px"}}>
                                        <ul>
                                            <li>
                                                <Link to="#">
                                                    <h4>Haircare</h4>
                                                </Link>
                                            </li>
                                            <li><Link to="#"> Shampoo</Link></li>
                                            <li><Link to="#"> Conditioner</Link></li>
                                            <li><Link to="#"> Hair Cream</Link></li>
                                            <li><Link to="#"> Hair Oil</Link></li>

                                            <li><Link to="#"> Hair Gel</Link></li>
                                            <li><Link to="#"> Hair Color</Link></li>
                                            <li><Link to="#"> Hair Serum</Link></li>
                                            <li><Link to="#"> Hair Accessory</Link></li>
                                        </ul>
                                    </div>
                                    <div style={{marginTop:"-40px"}}>
                                        <ul>
                                            <li>
                                                <Link to="#">
                                                    <h4>Appliances</h4>
                                                </Link>
                                            </li>
                                            <li><Link to="#"> Hair Straightener</Link></li>
                                            <li><Link to="#"> Hair Dryer</Link></li>
                                            <li><Link to="#"> Epilator</Link></li>

                                            <li><Link to="#"> Mascara</Link></li>

                                            <li><Link to="#"> Eyeliner</Link></li>
                                            <li><Link to="#"> Kajal</Link></li>
                                            <li><Link to="#"> Eyeshadow</Link></li>
                                            <li><Link to="#"> Foundation</Link></li>
                                            <li><Link to="#"> Primer</Link></li>
                                            <li><Link to="#"> Concealer</Link></li>
                                            <li><Link to="#"> Compact</Link></li>
                                            <li><Link to="#"> Nail Polish</Link></li>
                                        </ul>
                                    </div>
                                </div> */}
                            </li>
                            <li>
                                <span style={{padding:"26px 0px"}} id="studio-tag">STUDIO</span> <span className="studio-new">NEW</span>
                                <div className="bcg"></div>
                                {/* <div className="sub-menu-5" style={{width: "500px", marginLeft: "-300px", backgroundColor: "white"}}>

                                    <ul>
                                        <li>
                                            <Link to="#"></Link>
                                        </li>

                                        <li id="studio-logo-for"><Link to="#"> <img
                                            src="https://constant.myntassets.com/web/assets/img/studio-logo-new.svg" alt=""/> </Link></li>
                                        <li id="text-for-studio"><Link to="">Your daily inspiration for everything fashion</Link></li>
                                        <li id="start-img"><Link to="#"><img
                                            src="https://constant.myntassets.com/web/assets/img/sudio-nav-banner.png" alt=""/></Link></li>
                                        <li><Link to=""><button>Explore Studio</button></Link></li>

                                    </ul>
                                </div> */}
                            </li>
                        </div>





                        <input id="searchItem" className="icon" type="text" placeholder="Search for products, brands and more" />
                        <div id="Profile-Div">
                            <li>
                                <Link to="">
                                    <div className="pro">
                                        <img src="https://cdn-icons-png.flaticon.com/512/747/747376.png" alt="" />
                                        <span id="profile-tag">Profile</span>
                                    </div>

                                    <div id="profile-dropdown" className="sub-menu-1" style={{width: "300px", height: "316px", marginLeft: "-130px"}}>


                                        <ul id="profile-dropdown-hover">
                                            <div>
                                                <h3>Welcome</h3>
                                                <p style={{marginTop:"-15px "}}>To access account and manage orders</p>
                                             
                                            </div>
                                            <div style={{color: "#3e4152c5"}}>
                                                <li>Order</li>
                                                <li>Whishlist</li>
                                                <li>Gift Card</li>
                                                <li>Contact US</li>
                                                <li>Myntra Insider</li>

                                            </div>
                                            <div style={{color: "#3e4152c5"}}>
                                                <li>Myntra Credit</li>
                                                <li>Coupons</li>
                                                <li>Saved Cards</li>
                                                <li>Saved Addresses</li>

                                            </div>
                                        </ul>


                                    </div>


                                </Link>
                            </li>
                            <li>
                                <Link to="">
                                    <div className="pro">
                                        <img src="https://cdn-icons-png.flaticon.com/512/1077/1077035.png" alt="" />
                                        <span style={{marginLeft: "-2px"}}>Wishlist</span>
                                    </div>
                                </Link>
                            </li>
                            <li>
                                <Link to="/cart">
                                    <div id="last-bag-box" className="pro">
                                        <img src="https://cdn-icons-png.flaticon.com/512/1656/1656850.png" alt="" />
                                        <span id="cartCurrentItemShow" data-reactid="901" style={{fontSize: "11px", marginLeft: "10px"}}></span>
                                        <span style={{marginLeft: "-4px", marginTop: "-3px"}}>Bag</span>
                                    </div>
                                </Link>
                            </li>
                        </div>
                    </ul>
                </div>
            </div>
        </>
    );
}

export default NavBar